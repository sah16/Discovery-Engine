...
async def synthesize_insights(query: str, retrieved_context: List[dict], metrics: dict) -> dict:
    ...
    Generate a structured JSON output with FIVE keys:
    1. "is_out_of_scope": A boolean indicating if the query is unrelated to the product (true) or valid (false).
    2. "executive_summary": A string containing a 2-3 sentence conversational answer to the user's analytical query. If the query is framed as a question seeking understanding (e.g., "How do users..."), directly answer it based on the evidence. If, and ONLY if, EVERY SINGLE retrieved thread is deemed completely irrelevant to the query and you have zero evidence to draw from, output exactly: "No relevant feedback data was found for this query in the current dataset." If the query is just a vague keyword string or topic (e.g., "search", "error"), output null or an empty string.
    3. "retrieval_problems": A list of objects. Each object must have a "title" and "description". Limit to the top 3-4 problems. Ensure descriptions include relevant quantitative metrics.
    4. "opportunity_areas": A list of objects. For EVERY retrieval problem, provide a corresponding opportunity area object with a "title" and "description". The description MUST propose specific, actionable product features, UI changes, or algorithmic improvements. DO NOT generate vague conceptual statements (e.g., "improve search").
    5. "evidence": A list of objects, each containing a direct "quote" extracted EXACTLY from the text, and the "source_thread_id" (integer) it came from.
